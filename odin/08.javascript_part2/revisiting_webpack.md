...
We _really_ do not need to know exactly what optimizations have been applied, nor do we need to know about any other specifics of production mode, but it’s nice to be aware that the two modes exist and are designed for specific things.
...
Inside the `"scripts"` property and through a `bash` command.
...
It is a repository that serves as a template for new projects. This is useful to minimize copy and pasting.
